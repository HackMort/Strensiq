...
$(function () {
    ...
    if (window.innerWidth <= 1023) {
        ...
        setTimeout(function () {
            imgZoom('.starter-kit__img-container picture');
        }, 500);
    }
    window.addEventListener("resize", function () {
        if (window.innerWidth <= 1023) {
...
$(function () {
    ...
    function subNavFunction() {
        function f() {
            ...
            inViewScroll(href).on('enter', function (ele) {
                ...
                // mobile floating subnav
                $('.floating-subnav .floating-subnav__menu a').removeClass("active");
                $('.floating-subnav .floating-subnav__menu a[href="' + href + '"]').addClass("active");
            });
        }
        // Loop through all of the links within the class
        $('.pagesubnavcntr .js-accordion__panel a').each(f);
        $('.floating-subnav .floating-subnav__menu a').each(f);
    }
    $(document).ready(function () {
        subNavFunction();
    });
    ...
    $(window).scroll(function () {
        if ($('.isi-inPageHeader').length > 0) {
            if (window.innerWidth >= 1024) {
                if ($subNav.length) {
                    var scrollToSubnav = $subNav.offset().top + 60;
                }
                var isiInPageHeader = $('.isi-inPageHeader').offset().top + 0;
            } else {
                if ($subNav.length) {
                    var subnavHeight = $('[data-module="PageSubnav"]').outerHeight();
                    var scrollToSubnav = $subNav.offset().top + subnavHeight;
                }
                var isiInPageHeader = $('.isi-inPageHeader').offset().top;
            }
            if ($stickPoint.length && window.innerWidth >= 1024) {

                if ($(window).scrollTop() < scrollToSubnav) {
                    $stickPoint.css('display', 'block')
                    $stickPoint.removeClass('scrolled')

                } else if (($(window).scrollTop() >= scrollToSubnav) && ($(window).scrollTop() < isiInPageHeader)) {
                    $stickPoint.css('display', 'block')
                    $stickPoint.addClass('scrolled')
                } else {

                    $stickPoint.css('display', 'none')
                    $stickPoint.removeClass('scrolled')
                }
            }
        }
